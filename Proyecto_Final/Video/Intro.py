...
class Presentacion(Scene):
    def construct(self):
        ...
        #) as tracker:
        self.play(FadeIn(udg), run_time=1) 
        self.play(udg.animate.shift(LEFT), AnimationGroup(*anim_cucei, lag_ratio=0.2), run_time=1) # run_time=tracker.duration/3
        self.play(Write(nombre), run_time=1)
        ...
        #) as tracker:
        self.play(AnimationGroup(*anim_mov, lag_ratio=0.1), run_time=1)  # tracker.duration/4
        self.play(Transform(nombre, titulo), run_time=1)  # tracker.duration/4
...
class Indice(Scene):
    def construct(self):
        ...
        for line in lista:
            ...
            self.wait(2)  # tracker.duration-1.5